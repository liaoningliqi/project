...
assertion_info_t  assertion_info = {0};

static TaskHandle_t xHandleTaskUt;
...
uint32_t invoke_evt_cb(platform_evt_callback_type_t type, void *data, uint8_t lock)
{
...
void invoke_putc(int ch)
{
    invoke_evt_cb(PLATFORM_CB_EVT_PUTC, &ch, 0);
}
...
void free_rtos_transp_init()
{
    __disable_irq();
    NVIC_SetVectorTable(0x00, 0x4000);
    irq_setup();
    platform_set_evt_callback(PLATFORM_CB_EVT_PUTC, (f_platform_evt_cb)cb_putc, NULL);
    xTaskCreate(ut_task, "ut", 1024, NULL, (1), &xHandleTaskUt);
    return;
}
